...
def terminal_stats_excel(id):
    ...
    writer.save()
    output.seek(0)
    return send_file(output, attachment_filename='Выгрузка_{terminal}_{date}.xls'.format(terminal = terminal.description, date = datetime.datetime.now().strftime("%d_%m_%y") ), as_attachment=True, cache_timeout=0 )
