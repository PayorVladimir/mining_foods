    ...
    $('#groups-combo').combobox('reload', '/api/v1/groups_all');
          ...
          var dgterminals = $('#dg-terminals').datagrid({
                ...
                onExpandRow: function(index,row){
                    ...
                    ddv.datagrid({
                        url:'api/v1/logs?terminal_id='+row.terminal_id,
                        fitColumns:true,
                        singleSelect:true,
                        rownumbers:true,
                        remoteFilter: true,
...
function clientLogs(){
    var row = $('#dg').datagrid('getSelected');
     $('#client-logs').dialog('open').dialog('center').dialog('setTitle', 'Статистика: '+row.client_name);
        $('#logs').datagrid({url:'api/v1/logs?client_id='+row.client_id});
...
function editUser() {
    ...
    if (row) {
        ...
        $('#fm').form('load', row);
        url = '/api/v1/client/' + row.client_id;
        type = 'PUT';
...
function activateUser() {
     ...
     var row = $('#dg').datagrid('getSelected');
    var url = "/api/v1/activate_client";
    if (row) {
...
function activateTerminal() {
    var row = $('#dg-terminals').datagrid('getSelected');
    var url = "/api/v1/activate_terminal";
    if (row) {
...
function deleteTerminal() {
    var row = $('#dg-terminals').datagrid('getSelected');
    var url = "/api/v1/delete_terminal";
    if (row) {
...
function editTerminal() {
    ...
    if (row) {
        ...
        url = '/api/v1/terminals/'+ row.terminal_id;
...
function newGroup() {
    ...
    url = '/api/v1/new_group';
    type = 'POST';
}

function blockGroup() {
     var row = $('#dg-groups').datagrid('getSelected');
    var url = "/api/v1/block_group";
    if (row) {
...
function editGroup() {
    ...
    if (row) {
        ...
        url = '/api/v1/groups/' + row.group_id;
        type = 'PUT';
